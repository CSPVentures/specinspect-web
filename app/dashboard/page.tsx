...
import type { User, Project, SubscriptionStatus } from '@/lib/types';
...
export default async function DashboardPage() {
  ...
  if (!userRes.ok) redirect('/login?next=/dashboard');

  const user: User = await userRes.json().then((b) => b.user ?? b);
  const sub: SubscriptionStatus = subRes.ok ? await subRes.json().catch(() => ({ tier: 'free', status: 'none' })) : { tier: 'free', status: 'none' };
  const projectsBody = projectsRes.ok ? await projectsRes.json().catch(() => null) : null;
  const projects: Project[] = projectsBody
